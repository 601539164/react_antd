...
export default function UserEditForm({ editType, dict_id, dict, onRefreshTable, toggleModalStatus }) {
  // 表单组件实例
  ...
  const onFinish = async (value) => {
    ...
    switch (editType) {
      ...
      case 'edit':
        ...
        break
      // case 'reset':
      //   await userApi.manage.reset({ dict_id, ...value })
  ...
  return (
    <Form form={form} layout="vertical" name="userInfoForm" onFinish={onFinish}>
      {editType !== 'reset' && (
        <>
          <Form.Item name="dict_name" label="名称" rules={[{ required: true, type: 'string', min: 1, max: 10 }]}>
            <Input />
          </Form.Item>
          <Form.Item name="dict_code" label="code" rules={[{ required: true, type: 'string', min: 1, max: 10 }]}>
            <Input />
          </Form.Item>
          {/* <Form.Item
            name="role_ids"
          ...
          <Form.Item name="description" label="描述" rules={[{ required: true, type: 'string' }]}>
            <Input />
          </Form.Item>
        </>
      )}
